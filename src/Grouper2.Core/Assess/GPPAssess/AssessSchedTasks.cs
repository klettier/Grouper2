...
namespace Grouper2.Core.GPPAssess
{
    public partial class AssessGpp
    {
        ...
        private JObject GetAssessedScheduledTasks(JObject gppCategory, bool onlineChecks, int intLevelToShow)
        {
            ...
            foreach (string schedTaskType in schedTaskTypes)
            {
                if (gppCategory[schedTaskType] is JArray)
                {
                    ...
                    {
                        JObject assessedGppSchedTask = GetAssessedScheduledTask(taskJToken, schedTaskType, onlineChecks, intLevelToShow);
                ...
                {
                    JObject assessedGppSchedTask = GetAssessedScheduledTask(gppCategory[schedTaskType], schedTaskType, onlineChecks, intLevelToShow);
                    if (assessedGppSchedTask != null)
                    {
        ...
        private JProperty ExtractCommandFromScheduledTask(JToken scheduledTask, ref int interestLevel, int number = 0)
        {
            ...
            command = investigatePath(commandString);
            arguments = investigateString(argumentsString);
        ...
        private JObject GetAssessedScheduledTask(JToken scheduledTask, string schedTaskType, bool onlineChecks, int intLevelToShow)
        {
            int interestLevel = 4;
            ...
            {
                assessedScheduledTask.Add("Action", JUtil.GetActionString(scheduledTask["Properties"]["@action"].ToString(), debugWrite));
                ...
                if (scheduledTask["Properties"]["Task"]["Actions"]["SendEmail"] != null)
                {
                    ...
                    if (onlineChecks)
                    {
                        attachment = investigateString(attachmentString);
            ...
            if (schedTaskType == "Task")
            {
                ...
                JObject assessedWorkingDir = investigatePath(JUtil.GetSafeString(scheduledTask["Properties"], "@startIn"));
