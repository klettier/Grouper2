...
namespace Grouper2.Core.GPPAssess
{
    ...
    {
        private JObject GetAssessedGroups(JObject gppCategory, int intLevelToShow)
        {
            ...
            if (gppCategory["Group"] != null)
            {
                if (gppCategory["Group"] is JArray)
                {
                    ...
                    {
                        JObject assessedGroup = GetAssessedGroup(gppGroup, intLevelToShow);
            ...
            if (gppCategory["User"] != null)
            {
                if (gppCategory["User"] is JArray)
                {
                    ...
                    {
                        JObject assessedUser = GetAssessedUser(gppUser, intLevelToShow);
        ...
        private JObject GetAssessedUser(JObject gppUser, int intLevelToShow)
        {
            ...
            userAction = JUtil.GetActionString(userAction, debugWrite);
        ...
        private JObject GetAssessedGroup(JObject gppGroup, int intLevelToShow)
        {
            int interestLevel = 3;
            ...
            string groupAction = JUtil.GetActionString(gppGroupProps["@action"].ToString(), debugWrite);
            ...
            if (gppGroupProps["Members"] != null)
            {
                if (!(gppGroupProps["Members"] is JValue))
                {
                    if (gppGroupProps["Members"]["Member"] != null)
                    {
                        ...
                        {
                            debugWrite("Something went squirrely with Group Memberships");
                            debugWrite(members.Type.ToString());
                            debugWrite(" " + membersType + " ");
                            debugWrite(members.ToString());
                        }
                    }
                }
            ...
            if ((interestLevel > intLevelToShow) && (assessedGroup.HasValues))
            {
        ...
        private JObject GetAssessedGroupMember(JToken member)
        {
            ...
            {
                string resolvedSID = getUserFromSid(memberSid);
