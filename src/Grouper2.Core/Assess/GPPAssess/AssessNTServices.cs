...
namespace Grouper2.Core.GPPAssess
{
    ...
    {
        private JObject GetAssessedNTServices(JObject gppCategory, int intLevelToShow)
        {
            ...
            if (gppCategory["NTService"] is JArray)
            {
                ...
                {
                    JProperty assessedGppNtService = AssessGppNTService(gppNtService, investigatePath, investigateString, intLevelToShow);
            ...
            {
                JProperty assessedGppNtService = AssessGppNTService(gppCategory["NTService"], investigatePath, investigateString, intLevelToShow);
                assessedGppNtServices.Add(assessedGppNtService);
            }
        ...
        static JProperty AssessGppNTService(
            JToken gppNtService,
            ...
            int intLevelToShow)
        {
            ...
            JObject gppNtServiceProgram = investigatePath(JUtil.GetSafeString(gppNtService["Properties"], "@program"));
            JObject gppNtServiceArgs = investigateString(JUtil.GetSafeString(gppNtService["Properties"], "@args"));
