...
namespace Grouper2.Core.InfAssess
{
    ...
    {
        public static JObject AssessRegKeys(JToken regKeys, int intLevelToShow, Func<string, SecurableObjectType, JObject> parseSddlString)
        {
            ...
            foreach (KeyValuePair<string, JToken> regKey in regKeysJObject)
            {
                ...
                // go parse the SDDL
                JObject parsedSddl = parseSddlString(sddl, SecurableObjectType.WindowsService);
                ...
                if (sddl.Length > 4)
                {
                    ...
                    if (parsedSddl["DACL"] != null)
                    {
                        ...
                        foreach (JProperty ace in parsedSddl["DACL"].Children())
                        {
                            ...
                            if (aceInterestLevel >= intLevelToShow)
                            {
