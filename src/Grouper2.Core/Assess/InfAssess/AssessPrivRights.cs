...
namespace Grouper2.Core.InfAssess
{
    ...
    {
        public static JObject AssessPrivRights(
            JToken privRights,
            int intLevelToShow,
            bool onlineChecks,
            Func<string> getDomainSid,
            JArray intPrivRights,
                        ...
                        if (privRight.Value is JArray)
                        {
                            foreach (JToken trusteeJToken in privRight.Value)
                            {
                                ...
                                if (interestLevel >= intLevelToShow)
                                {
                                    try
                                    {
                                        trustees.Add(GetTrustee(trusteeClean, getUserFromSid));
                                    }
                                    catch (Exception e)
                                    {
        ...
        static JProperty GetTrustee(string trustee, Func<string, string> getUserFromSid)
        {
            string displayName = "";
            // clean up the trustee SID


            string resolvedSid = getUserFromSid(trustee);
