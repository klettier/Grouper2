...
namespace Grouper2.Core.SddlParser
{
    public class Acl
    {
        ...
        public Acl(string acl, Func<string, Ace> toAce)
        {
            Raw = acl;
            ...
            if (begin != -1)
            {
                ...
                for (int end = begin; end < acl.Length; end++)
                {
                    ...
                    else if (acl[end] == Ace.EndToken)
                    {
                        ...
                        if (balance == 0)
                            aces.AddLast(toAce(acl.Substring(begin + 1, length)));
                            //aces.AddLast(new Ace(acl.Substring(begin + 1, length), debugWrite, type));
                    }
                    else if (balance <= 0)
                    {
            ...
            if (anyAces)
            {
                ...
                foreach (Ace ace in Aces)
                {
                    ...
                    foreach (string right in ace.Rights)
                    {
                        ...
                        else if ((boringRights.ContainsKey(right)) && (intLevelToShow > 0))
                        {
                            continue;
                        }
                        else if ((boringRights.ContainsKey(right)) && (intLevelToShow == 0))
                        ...
                        else
                        {
                            ...
                            debugWrite?.Invoke(right + " was not defined as either boring or interesting. Consider adding it to the dicts in Acl.cs?");
                            aceRightsJArray.Add(right);
                        }
                    }

                    string displayName = getUserFromSid(aceSidRaw);
